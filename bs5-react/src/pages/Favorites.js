                    ...
                    <table className="table table-light table-striped mt-4" style={{borderRadius: "20px", textAlign:"center", margin:"auto", minWidth:"470px", overflow:"hidden"}}>
                        <thead>
                            <tr className="row px-2">
                                <th className="col-1 col-md-1 indexClass">#</th>
                                <th className="col-2 col-md-2 dateClass">Date</th>
                                <th className="col-3 col-md-3 eventClass">Event</th>
                                <th className="col-2 col-md-2 categoryClass">Category</th>
                                <th className="col-2 col-md-3 venueClass">Venue</th>
                                <th className="col-2 col-md-1">Favorite</th>
                            </tr>
                        </thead>
                        <tbody>
                        {rows.map((eventId, index) => {
                            ...
                            return (
                                <tr className="row px-2" key={eventId}>
                                    <td className="col-1 col-md-1 indexClass"><b>{index+1}</b></td>
                                    <td className="col-2 col-md-2 dateClass">{val.date}</td>
                                    <td className="col-3 col-md-3 eventClass">{val.event}</td>
                                    <td className="col-2 col-md-2 categoryClass">{val.category}</td>
                                    <td className="col-2 col-md-3 venueClass">{val.venue}</td>
                                    <td className="col-2 col-md-1"><BsTrash onClick={() => handleDelete(eventId)} style={{cursor:"pointer"}}/></td>
                                </tr>
                            )
                          })}
                        </tbody>
